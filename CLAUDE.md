...
**Pricing** (`src/pricing.json`, `src/pricing_grok.json`, `src/pricing_codex.json`) is a static local copy, not fetched live - `usage.js`'s `costForUsage` returns the real token breakdown with `cost: null` (not a guess, and not dropping the tokens) for a model missing from the tables; it only returns `null` outright when there's no usage to report at all. Codex usage is normalized into the shared accounting schema, while unknown Codex model IDs remain on this "tokens real, cost unpriced" path rather than being guessed.
